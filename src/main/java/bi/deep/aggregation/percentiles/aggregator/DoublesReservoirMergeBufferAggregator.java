...
public class DoublesReservoirMergeBufferAggregator implements BufferAggregator {
    private final ColumnValueSelector<DoublesReservoir> selector;
    private final IdentityHashMap<ByteBuffer, Int2ObjectMap<DoublesReservoir>> cache = new IdentityHashMap<>();
    private final int maxSize;

    public DoublesReservoirMergeBufferAggregator(ColumnValueSelector<DoublesReservoir> selector, int maxReservoirSize) {
    ...
    public void aggregate(ByteBuffer buffer, int position) {
        ...
        DoublesReservoir other = selector.getObject();

        if (other != null) {
            DoublesReservoir doublesReservoir = cache.get(buffer).get(position);
            doublesReservoir.merge(other);
        }
    }
