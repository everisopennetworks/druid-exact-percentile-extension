...
class DoublesReservoirToSUMPostAggregatorTest {
    @Test
    void testSerde() throws JsonProcessingException {
        final PostAggregator there = new DoublesReservoirToSUMPostAggregator(
                "post", new FieldAccessPostAggregator("field1", "reservoir"));
        DefaultObjectMapper mapper = new DefaultObjectMapper();
        DoublesReservoirToSUMPostAggregator andBackAgain =
                mapper.readValue(mapper.writeValueAsString(there), DoublesReservoirToSUMPostAggregator.class);
    ...
    @Test
    void testToString() {
        final PostAggregator postAgg = new DoublesReservoirToSUMPostAggregator(
                "post", new FieldAccessPostAggregator("field1", "reservoir"));

        assertEquals(
                "DoublesReservoirToSUMPostAggregator{name='post', field=FieldAccessPostAggregator{name='field1', fieldName='reservoir'}}",
                postAgg.toString());
    }
    ...
    public void emptyReservoir() {
        ...
        try (final Aggregator agg = new DoublesReservoirBuildAggregator(selector, 8)) {
            ...
            final PostAggregator postAgg = new DoublesReservoirToSUMPostAggregator(
                    "sum", new FieldAccessPostAggregator("field", "reservoir"));

            final Double sum = (Double) postAgg.compute(fields);
            assertNotNull(sum);
    ...
    public void testResultArraySignature() {
        final TimeseriesQuery query = Druids.newTimeseriesQueryBuilder()
                ...
                .postAggregators(new DoublesReservoirToSUMPostAggregator(
    ...
    void testEqualsAndHashCode() {
        EqualsVerifier.forClass(DoublesReservoirToSUMPostAggregator.class)
